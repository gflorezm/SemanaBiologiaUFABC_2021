...
> Olá. 
> Todos os dados podem ter anomalías como resultado de erros de digitação ou formatação. Em esta prática aprenderemos como ler, manipular e corregir algumas anomalías de nossos dados.

$~$

No repositório de [github](https://github.com/gflorezm/SemanaBiologiaUFABC_2021), dentro da pasta `Dados` estão depositados todos os arquivos de dados e um arquivo de metadados, nos metadados está a explicação detalhada da estrutura dos dados que usaremos no curso.
...
Agora vamos trabalhar com o conjunto de dados chamado `tennis.csv`. Usaremos o link da fonte porém, desta vez iremos salvá-los como um *data frame* no nosso ambiente de trabalho (não no diretório de nossa PC).
...
Perceba que usamos uma função distinta `read.csv()` e demos para ela o link da fonte e o argumento `sep = ";"`. Este último indica que o separador das colunas é um ponto e vírgula e não só uma vírgula (opção por defeito).

$~$

Agora vamos explorar de forma geral os dados. As funções `head()` e `tail()` devolvem as primeiras seis linhas e as ulítmas seis linhas do *data frame*. A função `View()` abre o *data frame* inteiro em uma aba ao lado do script.
...
Vamos explorar os reumos dos dados usando as funções `summary()` e `str()`. Vocês conseguem diferenciar as saídas dessas funções?
...
Podemos ver que por algúm motivo, a variável `pos_test` é de tipo `character`quando devería ser numérica. Vamos explorar os valores dessa variável usando a função `unique()`
...
Já percebemos as anomalías dos dados, a pessoa que transcreveu as planilhas colocou *"Nao"* nas células onde não havía dados (dados faltantes). Como o R reconheceu texto no meio dessa coluna a transformou em tipo `character`. Vamos corregir isso substituindo todos os "Nao" por "NA".
...
Perceba que há um erro: `tratamento` em alguma das colunas. Vamos primeiro ver onde está esse erro para corrigî-lo e depois transformar a variável `grupo` em um **Fator** já que por enquanto ela é só uma variável que contem texto mas esse texto precisa ser organizado na forma de uma variável categórica.
...
Agora é turno de vocês ler e corregir os dados do arquivo `a.lituratus` que se encontram [neste link](https://raw.githubusercontent.com/gflorezm/SemanaBiologiaUFABC_2021/main/Dados/a.lituratus.csv). 

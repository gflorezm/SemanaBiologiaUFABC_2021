...
author: "Guillermo Florez Montero e Otávio Marchetti"
date: "`Julho de 2021`"
output:
...
> Em esta prática vamos usar simulações para estudar um pouco da [Teoría de grandes amostras](https://pt.wikipedia.org/wiki/Teorema_central_do_limite) e como é utilizada na inferência estatística.
>
> De acordo com o Teorema Central do Limite, na medida que aumenta o tamanho da amostra, a distribuição das medias aproxima-se a uma distribuição Gaussiana. Vamos testar essa ideia com dados simulados de populações com distribuição Gaussiana e Uniforme.

$~$

## Amostrando uma população Gaussiana

Primeiro vamos simular um universo estatístico onde existe uma variável só, com média $\mu = 5$ e desvío padrão $\sigma = 2.5$. Este universo está formado por 5 milhoes de unidades de observação.
...
Vamos explorar essa variável de forma gráfica, construindo um histograma e adicionamos a média como uma linha vertical (lembre-se de rodar o código linha por linha para ver a saída de cada função).
...
Calcule a média e o desvío padrão usando as funções `mean()` e `sd()`. Veja que não são exatamente 5 e 2.5 que foram os valores exatos que usamos para a simulação, **por quê?**
...
Calcule a média e o desvío padrão da amostra.

$~$

Agora vamos fazer essa amostragem acontecer 200 vezes. Imaginem que fizemos o experimento (ou saímos ao campo) 200 vezes em momentos distintos para pegar 150 unidades de observação...
...
Podem explorar as amostras a vontade mas só lembrar que é uma matriz enorme. Use a função `dim()` aplicada ao objeto `amostras_1` para conferir o tamanho, a saída deveriam ser dois valores com o número de linhas e colunas.

$~$

Calculemos a média de cada uma das amostras usando a função `apply()` com três argumentos: 1) o objeto ( que é uma matriz), 2) a direção onde vamos calcular (1 para linhas e 2 para colunas) e 3) a função que iremos aplicar na matríz. Podem calcular os desvíos usando a mesma função mas trocando o último argumento por `sd`.
...
Grafiquemos a distribuição das 200 médias amostrais e vamos ajustar uma curva Gaussiana.
...
A distribuição das médias amostrais é, de fato, Gaussiana. Agora vamos estimar a média. Vamos calcular a média das médias (a grande média) e compará-la com a média do meu universo amostral, aquela que na realidade é impossível conhecer mas que agora conhecemos porque nós simulamos este universo. 
...
Para nada Gaussiano, não é? Agora vamos fazer a amostragem e ver se se cumpre o Teorema Central do Limite.
...
E finalmente graficamos a distribuição das 200 médias amostrais e ajustamos uma curva Gaussiana.
...
Parabéns, acabamos de comprobar o teorema central do limite. E aprendemos duas coisas importantes:
...
2) A estimativa dos parámetros é mais acurada quanto maior seja o tamanho amostral.

$~$

> Aquí é necessário fazer um paréntese para sair dos universos criados e voltar à realidade. Quando fazemos experimentos ou vamos ao campo na vida real, é muito difícil obter amostras repetidas de tamanhos grandes (*n > 100*). Sempre temos essa demanda de conflitos (sendo muito otimista) entre a) fazer varias amostragens com n pequeno ou b) fazer uma amostragem com n mais ou menos grande. O melhor que podemos fazer para lidar com isso é utilizar a **estatística** para fazer **inferência** sobre os nossos parámetros, usando modelos probabilísticos conhecidos.

$~$

## Inferência por intervalos de confiança

Vamoz fazer inferência da média de uma amostra construindo intervalos de confiança e usando como modelo probabilístico a distribuição [*t de Student*](https://pt.wikipedia.org/wiki/Distribui%C3%A7%C3%A3o_t_de_Student). Lembremos que esta distribuição é simétrica, com média $\bar{x} = 0$ e desvío padrão $s = 1$ porém, a forma da curva depende dos graus de liberdade (tamanho amostral). 
...
Esta distribuição tem sido estudada por mais de 100 anos, por tanto a probabilidade asociada a cada valor de para diferentes tamanhos amostrais é bem conhecido. Por exemplo, o 95% dos valores de uma distribuição com 20 graus de libertade estão entre `r round(qt(0.025, 20),3)` e `r round(qt(0.975, 20),3)`
...
* $s =$ desvío padrão amostral

* $n =$ tamanho amostral

Vamos aplicar essa equação criando uma função simples no R.

> A função é meio *fancy*, ela poderia ter sido mais rudimentar mas básicamente aceita dois argumentos:
>
> x = um vetor numérico
...
Para finalizar, repita o cálculo para diferentes amostras aumentando o valor de *n* em 100, 500, 1000, 1500 e veja o quê acontece com os intervalos de confiança. Mantenha o $\alpha = 0.05$.
